...
- [x] **Source citation panel** — every assistant message now carries a "N sources consulted" disclosure below it. Expands to per-source cards color-coded by kind (Quran emerald, Hadith amber, Tafsir sky, Bible violet) with similarity %. Click a card to reveal the actual retrieved chunk (up to 600 chars). Arabic-heavy chunks auto-flip to RTL with the Amiri font. The chat API now ships `content` in the message metadata to power this without an extra round-trip.
